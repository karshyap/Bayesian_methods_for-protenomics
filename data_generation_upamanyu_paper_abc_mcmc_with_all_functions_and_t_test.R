...
peptide_generation=function(gamma_vec,fold_change_vec,phi_opt,Npro_cntrl,Npep_cntrl,Npro_trt,Npep_trt,prot_pept_list_cntrl,Npro_for_analysis,
                            prot_pept_list_cntrl_test,prot_pept_list_trt,prot_pept_list_trt_test,train_test_flag)
{
  ...
  for(i in 1:Npro_cntrl){
    gamma_l_opt[i]=sample(gamma_vec,1)
  }
  #print(gamma_l_opt)
  gamma_l_opt=gamma_vec
...
########The peptide generating function ends here ########
...
main_function=function(pro_trt_file,pep_trt_file,pro_cntrl_file,pep_cntrl_file,noise_factor,theta_a,number_of_samples_train_cntrl,number_of_samples_train_trt,
                       number_of_samples_test_cntrl,number_of_samples_test_trt,amin,amax,k0,theta0,phi0,al0,M_cal,markov_iter,t_test_flag,Npro_factor)
{
  #noise_factor=1500
  #print(getwd())
  print("popo123")
  ...
  #####Everything related to t-test business#####
  ...
  if(1){
    ...
    t_test_value=t_test_value_pro
    
    pro_pept_listed=1:Npro_cntrl
    pro_list=pro_pept_listed[order(t_test_value,decreasing=TRUE)][1:Npro_for_analysis]
    
    gamma_l_opt=gamma_l_opt[order(t_test_value,decreasing=TRUE)][1:Npro_for_analysis]
    ...
    diff_rand_vec_cntrl=colMeans(c_pro_control_rand)-colMeans(c_pro_control0)
    print("pringleaaaaaaaaa")
    print(diff_rand_vec_cntrl)
    print("mringleaaaaaaaaa")
    ...
    diff_rand_vec_trt=colMeans(c_pro_treatement_rand)-colMeans(c_pro_treatement0)
    thresh_key_trt=1*euc_norm(diff_rand_vec_trt)
    
    
    #####Threshold generation ends####
  ...
  #print(c_pro_control0)
  print(pro_list)
  c_pro_control0=c_pro_control0[,pro_list]
  #ratio_vec=colMeans(c_pro_treatement0)/colMeans(c_pro_control0)
  #fold_change_vec=ratio_vec
  
  c_pro_control_0=c_pro_control0
  #print(c_pro_control_0)
  print(Npro)
  ...
  for(j in 1:markov_iter){
    ...
    #print("glishan")
    #print(mean_vec_cntrl_markov)
    #print(sig_matrix_markov)
    ...
    if(norm_cntrl_markov<(0.7*thresh_key_cntrl) & norm_trt_markov<(0.8*thresh_key_trt)){
      #mean_ratio=
      gamma_vec=colMeans(c_pro_control_markov)
      ###print(j)
  ...
  ####T-test related stuff####
  
  
  
  
  ####T-test related stuff ending
  ...
  #####This data is only for t_test feature generation
  #print(prot_pept_list_trt_test)
  xlj_all=peptide_generation(mean_vec_cont_opt,fold_change_vec,phi_opt,Npro_cntrl,Npep_cntrl,Npro_trt,Npep_trt,prot_pept_list_cntrl,Npro_for_analysis,
                             prot_pept_list_cntrl_test,prot_pept_list_trt,prot_pept_list_trt_test,train_test_flag)
  
  print("gnasher")
  
   train_test_flag=0
  xlj_all_test=peptide_generation(mean_vec_cont_opt,fold_change_vec,phi_opt,Npro_cntrl,Npep_cntrl,Npro_trt,Npep_trt,prot_pept_list_cntrl,Npro_for_analysis,
                                  prot_pept_list_cntrl_test,prot_pept_list_trt,prot_pept_list_trt_test,train_test_flag)
  ...
  if(t_test_flag){
    ...
    t_test_value=t_test_vals(xlj_all)
    cntrl_pro_list=1:Npro_cntrl
    q1=pro_list[order(t_test_value,decreasing=TRUE)][1:Npro_for_analysis]
    p1=prot_pept_list_cntrl
    prot_pept_list_cntrl=diff_elem_removal_total(p1,q1)
  ...
  markov_Data=peptide_generation(mean_vec_cont_opt,fold_change_vec,phi_opt,Npro_cntrl,Npep_cntrl,Npro_trt,Npep_trt,prot_pept_list_cntrl,Npro_for_analysis,
  ...
  if(1){
    for(i in 1:nrow(xlj_all_test)){
      ...
      for(j in (ceiling(0.4*markov_iter)):(markov_iter-1)){
        ...
        markov_Data=peptide_generation(gamma_markov[j,],fold_change_vec,phi_opt,Npro_cntrl,Npep_cntrl,Npro_trt,Npep_trt,prot_pept_list_cntrl,Npro_for_analysis,
                                       prot_pept_list_cntrl_test,prot_pept_list_trt,prot_pept_list_trt_test,train_test_flag = 1)
...
if(1){
  pro_trt_file="prot_file.fasta"
  pep_trt_file="pept_file.fasta"
}
pro_cntrl_file="prot_file.fasta"
pep_cntrl_file="pept_file.fasta"
noise_factor=1
theta_a=100
...
result_vec=c(0,0,0)
num_of_iter=5
for(l in 1:num_of_iter){

table_data_abc_mcmc=main_function(pro_trt_file,pep_trt_file,pro_cntrl_file,pep_cntrl_file,noise_factor,theta_a,number_of_samples_train_cntrl,number_of_samples_train_trt,
                                  number_of_samples_test_cntrl,number_of_samples_test_trt,amin,amax,k0,theta0,phi0,al0,M_cal,markov_iter,t_test_flag,Npro_factor = 0.15)

result_vec[1]=result_vec[1]+table_data_abc_mcmc[1]
result_vec[2]=result_vec[2]+table_data_abc_mcmc[2]
result_vec[3]=result_vec[3]+table_data_abc_mcmc[3]


}
#print(table_data_abc_mcmc)
print(result_vec/num_of_iter)